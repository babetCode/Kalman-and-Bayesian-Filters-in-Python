...
def _():
    ...
    dt_1, phi = symbols('\\Delta{t} \\Phi_s')
    _F_k = Matrix([[1, dt_1, dt_1 ** 2 / 2], [0, 1, dt_1], [0, 0, 1]])
    _Q_c = Matrix([[0, 0, 0], [0, 0, 0], [0, 0, 1]]) * phi
    _Q = integrate(_F_k * _Q_c * _F_k.T, (dt_1, 0, dt_1))
    _Q = _Q / phi
    MatMul(_Q, phi)
    return MatMul, Matrix, dt_1, integrate, phi, symbols
...
@app.cell
def _(Matrix, dt_1, integrate, phi):
    _F_k = Matrix([[1]])
    _Q_c = Matrix([[phi]])
    print('0th order discrete process noise')
    integrate(_F_k * _Q_c * _F_k.T, (dt_1, 0, dt_1))
    return


@app.cell
def _(MatMul, Matrix, dt_1, integrate, phi):
    _F_k = Matrix([[1, dt_1], [0, 1]])
    _Q_c = Matrix([[0, 0], [0, 1]]) * phi
    _Q = integrate(_F_k * _Q_c * _F_k.T, (dt_1, 0, dt_1))
...
@app.cell
def _(MatMul, Matrix, dt_1, symbols):
    _var = symbols('sigma^2_v')
    _v = Matrix([[dt_1 ** 2 / 2], [dt_1]])
    _Q = _v * _var * _v.T
    _Q = _Q / _var
    MatMul(_Q, _var)
    return
...
def _():
    ...
    _Q = Q_continuous_white_noise(dim=2, dt=1, spectral_density=1)
    print(_Q)
    return Q_continuous_white_noise, Q_discrete_white_noise


@app.cell
def _(Q_continuous_white_noise):
    _Q = Q_continuous_white_noise(dim=3, dt=1, spectral_density=1)
    print(_Q)
    return
...
@app.cell
def _(Q_discrete_white_noise):
    _Q = Q_discrete_white_noise(2, var=1.0)
    print(_Q)
    return
...
@app.cell
def _(Q_discrete_white_noise):
    _Q = Q_discrete_white_noise(3, var=1.0)
    print(_Q)
    return
...
@app.cell
def _(Q_continuous_white_noise, np):
    np.set_printoptions(precision=8)
    _Q = Q_continuous_white_noise(dim=3, dt=0.05, spectral_density=1)
    print(_Q)
...
def _(np):
    ...
    _t = np.linspace(-1, 1, 10)
    plt.plot(_t, np.exp(_t))
    _t = np.linspace(-1, 1, 2)
    plt.plot(_t, _t + 1, ls='--', c='k')
    return (plt,)
...
def _(np, plt):
    ...
    _t = np.linspace(-1, 2, 20)
    plt.plot(_t, np.exp(_t))
    _t = np.linspace(0, 1, 2)
...
@app.cell
def _(dx, np, plt):
    ys = euler(0, 4, 1, dx, step=1e-05)
    plt.subplot(1, 2, 1)
    plt.title('Computed')
    plt.plot(np.linspace(0, 4, len(ys)), ys)
    plt.subplot(1, 2, 2)
    _t = np.linspace(0, 4, 20)
    plt.title('Exact')
    plt.plot(_t, np.exp(_t))
    return (ys,)
...
def _(np, plt):
    ...
    def func(y, t):
        return _t * math.sqrt(y)
    while _t <= 10:
        y = runge_kutta4(y, _t, dt_2, func)
        _t = _t + dt_2
        ys_1.append(y)
        ts.append(_t)
    exact = [(_t ** 2 + 4) ** 2 / 16.0 for _t in ts]
    plt.plot(ts, ys_1)
    plt.plot(ts, exact)
    error = np.array(exact) - np.array(ys_1)
