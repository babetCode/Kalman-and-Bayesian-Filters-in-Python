...
def _(np):
    ...
    _mean = (0.0, 0.0)
    _p = np.array([[32.0, 15.0], [15.0, 40.0]])
    mean_fx = f_nonlinear_xy(*_mean)
    xs, _ys = multivariate_normal(mean=_mean, cov=_p, size=10000).T
    plot_monte_carlo_mean(xs, _ys, f_nonlinear_xy, mean_fx, 'Linearized Mean')
    return f_nonlinear_xy, multivariate_normal, plot_monte_carlo_mean
...
@app.cell
def _(fx, hx, plt, sigmas):
    from numpy.random import randn
    from filterpy.kalman import UnscentedKalmanFilter
    from filterpy.common import Q_discrete_white_noise
    _ukf = UnscentedKalmanFilter(dim_x=2, dim_z=1, dt=1.0, hx=hx, fx=fx, points=sigmas)
    _ukf.P = _ukf.P * 10
    _ukf.R = _ukf.R * 0.5
    _ukf.Q = Q_discrete_white_noise(2, dt=1.0, var=0.03)
    zs, xs_1 = ([], [])
    for _i in range(50):
        _z = _i + randn() * 0.5
        _ukf.predict()
        _ukf.update(_z)
        xs_1.append(_ukf.x[0])
        zs.append(_z)
    plt.plot(xs_1)
    plt.plot(zs, marker='x', ls='')
    return Q_discrete_white_noise, randn
...
@app.cell
def _(f_nonlinear_xy, multivariate_normal, np, plot_monte_carlo_mean, plt):
    from filterpy.kalman import unscented_transform, MerweScaledSigmaPoints
    import scipy.stats as stats
    _mean = (0.0, 0.0)
    _p = np.array([[32.0, 15], [15.0, 40.0]])
    _points = MerweScaledSigmaPoints(n=2, alpha=0.3, beta=2.0, kappa=0.1)
    sigmas_1 = _points.sigma_points(_mean, _p)
    sigmas_f = np.empty((5, 2))
    for _i in range(5):
        sigmas_f[_i] = f_nonlinear_xy(sigmas_1[_i, 0], sigmas_1[_i, 1])
    ukf_mean, ukf_cov = unscented_transform(sigmas_f, _points.Wm, _points.Wc)
    np.random.seed(100)
    xs_2, _ys = multivariate_normal(mean=_mean, cov=_p, size=5000).T
    plot_monte_carlo_mean(xs_2, _ys, f_nonlinear_xy, ukf_mean, 'Unscented Mean')
    ax = plt.gcf().axes[0]
    ax.scatter(sigmas_1[:, 0], sigmas_1[:, 1], c='r', s=30)
...
@app.cell
def _(Q_discrete_white_noise, np, plt, randn):
    ...
    zs_1 = [np.array([_i + randn() * std_x, _i + randn() * std_y]) for _i in range(100)]
    xs_3, _, _, _ = kf.batch_filter(zs_1)
    plt.plot(xs_3[:, 0], xs_3[:, 2])
    return dt, xs_3, zs_1
...
def _(
    ...
    from filterpy.kalman import UnscentedKalmanFilter as UKF
    sigmas_2 = MerweScaledSigmaPoints(4, alpha=0.1, beta=2.0, kappa=1.0)
    _ukf = UKF(dim_x=4, dim_z=2, fx=f_cv, hx=h_cv, dt=dt, points=sigmas_2)
    _ukf.x = np.array([0.0, 0.0, 0.0, 0.0])
    _ukf.R = np.diag([0.09, 0.09])
    _ukf.Q[0:2, 0:2] = Q_discrete_white_noise(2, dt=1, var=0.02)
    _ukf.Q[2:4, 2:4] = Q_discrete_white_noise(2, dt=1, var=0.02)
    uxs = []
    for _z in zs_1:
        _ukf.predict()
        _ukf.update(_z)
        uxs.append(_ukf.x.copy())
    uxs = np.array(uxs)
    plt.plot(uxs[:, 0], uxs[:, 2])
    print(f'UKF standard deviation {np.std(uxs - xs_3):.3f} meters')
    return (UKF,)
...
def _(np, randn):
    ...
    class ACSim:
        ...
        def __init__(self, pos, vel, vel_std):
            self.pos = np.asarray(_pos, dtype=float)
    ...
    return ACSim, RadarStation, atan2
...
def _(
    ...
    _ac_vel = (100.0, 0.0)
    _radar_pos = (0.0, 0.0)
    h_radar.radar_pos = _radar_pos
    _points = MerweScaledSigmaPoints(n=3, alpha=0.1, beta=2.0, kappa=0.0)
    kf_1 = UKF(3, 2, dt_1, fx=f_radar, hx=h_radar, points=_points)
    kf_1.Q[0:2, 0:2] = Q_discrete_white_noise(2, dt=dt_1, var=0.1)
    kf_1.Q[2, 2] = 0.1
    kf_1.R = np.diag([range_std ** 2, elevation_angle_std ** 2])
    kf_1.x = np.array([0.0, 90.0, 1100.0])
    kf_1.P = np.diag([300 ** 2, 30 ** 2, 150 ** 2])
    np.random.seed(200)
    _pos = (0, 0)
    radar = RadarStation(_pos, range_std, elevation_angle_std)
    _ac = ACSim(ac_pos, (100, 0), 0.02)
    _time = np.arange(0, 360 + dt_1, dt_1)
    xs_4 = []
    for _ in _time:
        _ac.update(dt_1)
        _r = radar.noisy_reading(_ac.pos)
        kf_1.predict()
        kf_1.update([_r[0], _r[1]])
        xs_4.append(kf_1.x)
    plot_radar(xs_4, _time)
    return (
        ac_pos,
        dt_1,
        elevation_angle_std,
...
@app.cell
def _(ACSim, ac_pos, dt_1, kf_1, np, radar):
    ...
    for t in _time:
        ...
        _ac.update(dt_1)
        _r = radar.noisy_reading(_ac.pos)
        _ys.append(_ac.pos[1])
        kf_1.predict()
        kf_1.update([_r[0], _r[1]])
        xs_5.append(kf_1.x)
    plot_altitude(xs_5, _time, _ys)
    print(f'Actual altitude: {_ac.pos[1]:.1f}')
    print(f'UKF altitude   : {xs_5[-1][2]:.1f}')
...
@app.cell
def _(MerweScaledSigmaPoints, Q_discrete_white_noise, UKF, dt_1, np):
    def f_cv_radar(x, dt):
        """ state transition function for a constant velocity 
        aircraft"""
    ...
    def cv_UKF(fx, hx, R_std):
        _points = MerweScaledSigmaPoints(n=4, alpha=0.1, beta=2.0, kappa=-1.0)
        kf = UKF(4, len(R_std), dt_1, fx=fx, hx=hx, points=_points)
        kf.Q[0:2, 0:2] = Q_discrete_white_noise(2, dt=dt_1, var=0.1)
        kf.Q[2:4, 2:4] = Q_discrete_white_noise(2, dt=dt_1, var=0.1)
        kf.R = np.diag(R_std)
        kf.R = kf.R @ kf.R
...
def _(
    ...
    for t_1 in _time:
        ...
        _ys.append(_ac.pos[1])
        kf_cv.predict()
        kf_cv.update([_r[0], _r[1]])
        xs_6.append(kf_cv.x)
    plot_altitude(xs_6, _time, _ys)
    print(f'Actual altitude: {_ac.pos[1]:.1f}')
    print(f'UKF altitude   : {xs_6[-1][2]:.1f}')
    return
...
def _(
    ...
    range_std_1 = 500.0
    elevation_angle_std_1 = math.degrees(0.5)
    np.random.seed(200)
    _pos = (0, 0)
    radar_1 = RadarStation(_pos, range_std_1, elevation_angle_std_1)
    _ac = ACSim(ac_pos, (100, 0), 0.02)
    kf_sf = cv_UKF(f_cv_radar, h_radar, R_std=[range_std_1, elevation_angle_std_1])
    _time = np.arange(0, 360 + dt_1, dt_1)
    xs_7 = []
    for _ in _time:
        _ac.update(dt_1)
        _r = radar_1.noisy_reading(_ac.pos)
        kf_sf.predict()
        kf_sf.update([_r[0], _r[1]])
        xs_7.append(kf_sf.x)
    xs_7 = np.asarray(xs_7)
    plot_radar(xs_7, _time, plot_x=False, plot_vel=True, plot_alt=False)
    print(f'Velocity std {np.std(xs_7[10:, 1]):.1f} m/s')
    return
...
def _(
    ...
    h_vel.radar_pos = (0, 0)
    range_std_2 = 500.0
    elevation_angle_std_2 = math.degrees(0.5)
    vel_std = 2.0
    np.random.seed(200)
    _ac = ACSim(ac_pos, (100, 0), 0.02)
    radar_2 = RadarStation((0, 0), range_std_2, elevation_angle_std_2)
    kf_sf2 = cv_UKF(f_cv_radar, h_vel, R_std=[range_std_2, elevation_angle_std_2, vel_std, vel_std])
    _time = np.arange(0, 360 + dt_1, dt_1)
    xs_8 = []
    for t_2 in _time:
        _ac.update(dt_1)
        _r = radar_2.noisy_reading(_ac.pos)
        vx = _ac.vel[0] + randn() * vel_std
        vz = _ac.vel[1] + randn() * vel_std
        kf_sf2.predict()
        kf_sf2.update([_r[0], _r[1], vx, vz])
        xs_8.append(kf_sf2.x)
    xs_8 = np.asarray(xs_8)
    plot_radar(xs_8, _time, plot_x=False, plot_vel=True, plot_alt=False)
    print(f'Velocity std {np.std(xs_8[10:, 1]):.1f} m/s')
    return
...
def _(math):
    def bearing(sensor, target_pos):
        return math.atan2(_target_pos[1] - sensor[1], _target_pos[0] - sensor[0])
...
def _(bearing):
    def measurement(A_pos, B_pos, pos):
        angle_a = bearing(A_pos, _pos)
        angle_b = bearing(B_pos, _pos)
...
def _(measurement):
    ...
    def hx_VOR(x):
        _pos = (x[0], x[2])  # measurement to A
        return measurement(sa_pos, sb_pos, _pos)

    return hx_VOR, sa_pos, sb_pos
...
def _(
    ...
    def moving_target_filter(pos, std_noise, Q, dt=0.1, kappa=0.0):
        _points = MerweScaledSigmaPoints(n=4, alpha=0.1, beta=2.0, kappa=kappa)
        f = UKF(dim_x=4, dim_z=2, dt=dt, hx=hx_VOR, fx=fx_VOR, points=_points)
        f.x = np.array([_pos[0], 1.0, _pos[1], 1.0])
    ...
    def plot_straight_line_target(f, std_noise):
        xs, txs = ([], [])
        for _i in range(300):
            _target_pos[0] = _target_pos[0] + (1 + randn() * 0.0001)
            _target_pos[1] = _target_pos[1] + (1 + randn() * 0.0001)
            txs.append((_target_pos[0], _target_pos[1]))
            _z = measurement(sa_pos, sb_pos, _target_pos)
            _z[0] = _z[0] + randn() * std_noise
            _z[1] = _z[1] + randn() * std_noise
            f.predict()
            f.update(_z)
            xs.append(f.x)
        xs = np.asarray(xs)
    ...
    f_1 = moving_target_filter(_target_pos, std_noise, Q=1.0)
    plot_straight_line_target(f_1, std_noise)
    return moving_target_filter, plot_straight_line_target, std_noise
...
@app.cell
def _(moving_target_filter, plot_straight_line_target, std_noise):
    _target_pos = [0, 0]
    f_2 = moving_target_filter(_target_pos, std_noise, Q=1.0)
    plot_straight_line_target(f_2, std_noise)
    return
...
@app.cell
def _(MerweScaledSigmaPoints):
    _points = MerweScaledSigmaPoints(n=2, alpha=0.1, beta=2.0, kappa=1.0)
    _points.sigma_points(x=[0.0, 0], P=[[1.0, 0.1], [0.1, 1]])
    return
...
def _(np):
    import scipy
    _a = np.array([[2.0, 0.1], [0.1, 3]])
    s = scipy.linalg.cholesky(_a)
    print('cholesky:')
    print(s)
    print('\nsquare of cholesky:')
    print(s @ s.T)
...
def _(
    ...
    h_radar.radar_pos = _radar_pos
    _points = MerweScaledSigmaPoints(n=3, alpha=0.1, beta=2.0, kappa=0.0)
    kf_2 = UKF(3, 2, dt_2, fx=f_radar, hx=h_radar, points=_points)
    kf_2.Q[0:2, 0:2] = Q_discrete_white_noise(2, dt=dt_2, var=0.1)
    kf_2.Q[2, 2] = 0.1
    kf_2.R = np.diag([range_std_3 ** 2, bearing_std ** 2])
    kf_2.x = np.array([0.0, 90.0, 1100.0])
    kf_2.P = np.diag([300 ** 2, 30 ** 2, 150 ** 2])
    radar_3 = RadarStation((0, 0), range_std_3, bearing_std)
    _ac = ACSim(ac_pos_1, (100, 0), 0.02)
    np.random.seed(200)
    t_3 = np.arange(0, 360 + dt_2, dt_2)
    n = len(t_3)
    zs_2 = []
    for _i in range(len(t_3)):
        _ac.update(dt_2)
        _r = radar_3.noisy_reading(_ac.pos)
        zs_2.append([_r[0], _r[1]])
    xs_9, covs = kf_2.batch_filter(zs_2)
    ukf_internal.plot_radar(xs_9, t_3)
    return covs, kf_2, t_3, xs_9
...
@app.cell
def _(covs, kf_2, t_3, ukf_internal, xs_9):
    Ms, P, K = kf_2.rts_smoother(xs_9, covs)
    ukf_internal.plot_rts_output(xs_9, Ms, t_3)
    return
...
def _(np):
    ...
    def move(x, dt, u, wheelbase):
        ...
        if abs(steering_angle) > 0.001:
            beta = dist / _wheelbase * tan(steering_angle)  # is robot turning?
            _r = _wheelbase / tan(steering_angle)
            sinh, sinhb = (sin(hdg), sin(hdg + beta))  # radius
            cosh, coshb = (cos(hdg), cos(hdg + beta))
            return x + np.array([-_r * sinh + _r * sinhb, _r * cosh - _r * coshb, beta])
        else:
            return x + np.array([dist * cos(hdg), dist * sin(hdg), 0])  # moving in straight line
...
@app.cell
def _(normalize_angle):
    def residual_h(a, b):
        y = _a - b
        for _i in range(0, len(y), 2):  # data in format [dist_1, bearing_1, dist_2, bearing_2,...]
            y[_i + 1] = normalize_angle(y[_i + 1])
        return y
...
@app.cell
def _(atan2, normalize_angle, np, sqrt):
    def Hx(x, landmarks):
...
def _(atan2, np):
    ...
    def z_mean(sigmas, Wm):
        ...
        x = np.zeros(z_count)
        for _z in range(0, z_count, 2):
            sum_sin = np.sum(np.dot(np.sin(sigmas[:, _z + 1]), Wm))
            sum_cos = np.sum(np.dot(np.cos(sigmas[:, _z + 1]), Wm))
            x[_z] = np.sum(np.dot(sigmas[:, _z], Wm))
            x[_z + 1] = atan2(sum_sin, sum_cos)
        return x

    return state_mean, z_mean
...
def _(
    ...
    def run_localization(cmds, landmarks, sigma_vel, sigma_steer, sigma_range, sigma_bearing, ellipse_step=1, step=10):
        plt.figure()
        _points = MerweScaledSigmaPoints(n=3, alpha=1e-05, beta=2, kappa=0, subtract=residual_x)
        _ukf = UKF(dim_x=3, dim_z=2 * len(landmarks), fx=move, hx=Hx, dt=dt_3, points=_points, x_mean_fn=state_mean, z_mean_fn=z_mean, residual_x=residual_x, residual_z=residual_h)
        _ukf.x = np.array([2, 6, 0.3])
        _ukf.P = np.diag([0.1, 0.1, 0.05])
        _ukf.R = np.diag([sigma_range ** 2, sigma_bearing ** 2] * len(landmarks))
        _ukf.Q = np.eye(3) * 0.0001
        sim_pos = _ukf.x.copy()
        if len(landmarks) > 0:
            plt.scatter(landmarks[:, 0], landmarks[:, 1], marker='s', s=60)
        track = []
        for _i, u in enumerate(cmds):
            sim_pos = move(sim_pos, dt_3 / step, u, _wheelbase)
            track.append(sim_pos)
            if _i % step == 0:
                _ukf.predict(u=u, wheelbase=_wheelbase)
                if _i % ellipse_step == 0:
                    plot_covariance_ellipse((_ukf.x[0], _ukf.x[1]), _ukf.P[0:2, 0:2], std=6, facecolor='k', alpha=0.3)
                ...
                for lmark in landmarks:
                    ...
                    _a = normalize_angle(bearing - sim_pos[2] + randn() * sigma_bearing)
                    _z.extend([d, _a])
                _ukf.update(_z, landmarks=landmarks)
                if _i % ellipse_step == 0:
                    plot_covariance_ellipse((_ukf.x[0], _ukf.x[1]), _ukf.P[0:2, 0:2], std=6, facecolor='g', alpha=0.8)
...
@app.cell
def _(np, run_localization):
    landmarks = np.array([[5, 10], [10, 5], [15, 15]])
    cmds = [np.array([1.1, 0.01])] * 200
    _ukf = run_localization(cmds, landmarks, sigma_vel=0.1, sigma_steer=np.radians(1), sigma_range=0.3, sigma_bearing=0.1)
    print('Final P:', _ukf.P.diagonal())
    return
...
@app.cell
def _(np):
    landmarks_1 = np.array([[5, 10], [10, 5], [15, 15], [20, 5], [0, 30], [50, 30], [40, 10]])
    dt_4 = 0.1
    _wheelbase = 0.5
    sigma_range = 0.3
    sigma_bearing = 0.1
    ...
    def turn(v, t0, t1, steps):
        return [[v, _a] for _a in np.linspace(np.radians(t0), np.radians(t1), steps)]
    cmds_1 = [[v, 0.0] for v in np.linspace(0.001, 1.1, 30)]
    cmds_1.extend([cmds_1[-1]] * 50)
    v = cmds_1[-1][0]
    cmds_1.extend(turn(v, 0, 2, 15))
    cmds_1.extend([cmds_1[-1]] * 100)
    cmds_1.extend(turn(v, 2, -2, 15))
    cmds_1.extend([cmds_1[-1]] * 200)
    cmds_1.extend(turn(v, -2, 0, 15))
    cmds_1.extend([cmds_1[-1]] * 150)
    cmds_1.extend(turn(v, 0, 1, 25))
    cmds_1.extend([cmds_1[-1]] * 100)
    return cmds_1, landmarks_1


@app.cell
def _(cmds_1, landmarks_1, np, run_localization):
    _ukf = run_localization(cmds_1, landmarks_1, sigma_vel=0.1, sigma_steer=np.radians(1), sigma_range=0.3, sigma_bearing=0.1, step=1, ellipse_step=20)
    print('final covariance', _ukf.P.diagonal())
    return
...
@app.cell
def _(cmds_1, landmarks_1, np, run_localization):
    _ukf = run_localization(cmds_1, landmarks_1[0:2], sigma_vel=0.1, sigma_steer=np.radians(1), sigma_range=0.3, sigma_bearing=0.1, step=1, ellipse_step=20)
    print('final covariance', _ukf.P.diagonal())
    return
