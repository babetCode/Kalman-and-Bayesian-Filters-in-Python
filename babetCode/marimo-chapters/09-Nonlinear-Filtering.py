...
@app.cell
def _(data, np, plot_nonlinear_func):
    def g2(x):
...
def _(data, g2, plt):
    _N = 30000
    plt.subplot(121)
    plt.scatter(data[:_N], range(_N), alpha=0.1, s=1.5)
    plt.title('Input')
    plt.subplot(122)
    plt.title('Output')
    plt.scatter(g2(data[:_N]), range(_N), alpha=0.1, s=1.5)
    return
...
@app.cell
def _(data, g2, plot_nonlinear_func):
    y = g2(data)
...
def _(data, np, plot_nonlinear_func):
    ...
    _out = g3(data)
    print('output mean, variance: %.4f, %.4f' % (np.mean(_out), np.var(_out)))
    return (g3,)
...
def _(data, g2, g3, np):
    ...
    for i in range(10):
        _out = g3(_out)
        out2 = g2(out2)
    print('linear    output mean, variance: %.4f, %.4f' % (np.average(_out), np.std(_out) ** 2))
...
@app.cell
def _(normal, plot_nonlinear_func):
    def g3_1(x):
        return -x * x
    data_1 = normal(loc=1, scale=1, size=500000)
    plot_nonlinear_func(data_1, g3_1)
    return
