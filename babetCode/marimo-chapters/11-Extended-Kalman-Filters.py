...
def _():
    ...
    class RadarSim:
        ...
        def get_range(self):
            ...
            self.alt = self.alt + 0.1 * randn()  # add some process noise to the system
            self.pos = self.pos + self.vel * self.dt
            err = self.pos * 0.05 * randn()
            slant_dist = math.sqrt(self.pos ** 2 + self.alt ** 2)
            return slant_dist + err  # add measurement noise

    return RadarSim, randn
...
@app.cell
def _(HJacobian_at, RadarSim, ekf_internal):
    ...
    _dt = 0.05
    rk = ExtendedKalmanFilter(dim_x=3, dim_z=1)
    radar = RadarSim(_dt, pos=0.0, vel=100.0, alt=1000.0)
    rk.x = array([radar.pos - 100, radar.vel + 100, radar.alt + 1000])
    rk.F = eye(3) + array([[0, 1, 0], [0, 0, 0], [0, 0, 0]]) * _dt
    range_std = 5.0
    rk.R = np.diag([range_std ** 2])
    rk.Q[0:2, 0:2] = Q_discrete_white_noise(2, dt=_dt, var=0.1)
    rk.Q[2, 2] = 0.1
    rk.P = rk.P * 50
    xs, track = ([], [])
    for i in range(int(20 / _dt)):
        ...
        rk.update(array([_z]), HJacobian_at, hx)
        xs.append(rk.x)
        rk.predict()
    xs = asarray(xs)
    track = asarray(track)
    time = np.arange(0, len(xs) * _dt, _dt)
...
@app.cell
def _(sympy, x, y):
    from sympy.abc import alpha, v, w, R, theta
    ...
    r = w / sympy.tan(alpha)
    fxu = Matrix([[x - r * sympy.sin(theta) + r * sympy.sin(theta + beta)], [y + r * sympy.cos(theta) - r * sympy.cos(theta + beta)], [theta + beta]])
    F = fxu.jacobian(Matrix([x, y, theta]))
    F
    return F, Matrix, alpha, d, fxu, symbols, theta, time_1, v, w
...
@app.cell
def _(F, alpha, d, symbols, sympy, w):
    # reduce common expressions
    B, R_1 = symbols('beta, R')
    F_1 = F.subs(d / w * sympy.tan(alpha), B)
    F_1.subs(w / sympy.tan(alpha), R_1)
    return B, R_1
...
@app.cell
def _(B, Matrix, R_1, alpha, fxu, sympy, time_1, v, w):
    V = fxu.jacobian(Matrix([v, alpha]))
    V = V.subs(sympy.tan(alpha) / w, 1 / R_1)
    V = V.subs(time_1 * v / R_1, B)
    V = V.subs(time_1 * v, 'd')
    V
    return
...
@app.cell
def _(Matrix, symbols, sympy, theta, x, y):
    px, py = symbols('p_x, p_y')
    _z = Matrix([[sympy.sqrt((px - x) ** 2 + (py - y) ** 2)], [sympy.atan2(py - y, px - x) - theta]])
    _z.jacobian(Matrix([x, y, theta]))
    return
...
def _(array, sqrt):
    def H_of(x, landmark_pos):
        ...
        dist = sqrt(hyp)
        H = array([[-(px - x[0, 0]) / dist, -(py - x[1, 0]) / dist, 0], [(py - x[1, 0]) / hyp, -(px - x[0, 0]) / hyp, -1]])
        return H
...
@app.cell
def _(Matrix, array, cos, np, sin, symbols, sympy, tan):
    ...
    class RobotEKF(EKF):
        ...
        def __init__(self, dt, wheelbase, std_vel, std_steer):
            ...
            self.subs = {x: 0, y: 0, v: 0, a: 0, time: _dt, w: wheelbase, theta: 0}
        ...
        def move(self, x, u, dt):
            ...
            if abs(steering_angle) > 0.001:
                ...
                dx = np.array([[-r * sin(hdg) + r * sin(hdg + beta)], [r * cos(hdg) - r * cos(hdg + beta)], [beta]])
            else:
...
@app.cell
def _(H_of, Hx, RobotEKF, array, atan2, np, randn, residual):
    ...
    def z_landmark(lmark, sim_pos, std_rng, std_brg):
        ...
        _z = np.array([[d + randn() * std_rng], [a + randn() * std_brg]])
        return _z

    def ekf_update(ekf, z, landmark):
        _ekf.update(_z, HJacobian=H_of, Hx=Hx, residual=residual, args=landmark, hx_args=landmark)

    def run_localization(landmarks, std_vel, std_steer, std_range, std_bearing, step=10, ellipse_step=20, ylim=None):
        _ekf = RobotEKF(_dt, wheelbase=0.5, std_vel=std_vel, std_steer=std_steer)
        _ekf.x = array([[2, 6, 0.3]]).T
        _ekf.P = np.diag([0.1, 0.1, 0.1])
        _ekf.R = np.diag([std_range ** 2, std_bearing ** 2])
        sim_pos = _ekf.x.copy()
        u = array([1.1, 0.01])
        plt.figure()
        ...
        track = []
        for i in range(200):
            sim_pos = _ekf.move(sim_pos, u, _dt / 10.0)
            track.append(sim_pos)
            if i % step == 0:
                _ekf.predict(u=u)
                if i % ellipse_step == 0:
                    plot_covariance_ellipse((_ekf.x[0, 0], _ekf.x[1, 0]), _ekf.P[0:2, 0:2], std=6, facecolor='k', alpha=0.3)
                x, y = (sim_pos[0, 0], sim_pos[1, 0])
                for lmark in landmarks:
                    _z = z_landmark(lmark, sim_pos, std_range, std_bearing)
                    ekf_update(_ekf, _z, lmark)
                if i % ellipse_step == 0:
                    plot_covariance_ellipse((_ekf.x[0, 0], _ekf.x[1, 0]), _ekf.P[0:2, 0:2], std=6, facecolor='g', alpha=0.8)
    ...
    return cos, plt, run_localization, sin, tan


@app.cell
def _(array, np, run_localization):
    landmarks = array([[5, 10], [10, 5], [15, 15]])
    _ekf = run_localization(landmarks, std_vel=0.1, std_steer=np.radians(1), std_range=0.3, std_bearing=0.1)
    print('Final P:', _ekf.P.diagonal())
    return
...
@app.cell
def _(array, np, plt, run_localization):
    landmarks_1 = array([[5, 10], [10, 5], [15, 15], [20, 5]])
    _ekf = run_localization(landmarks_1, std_vel=0.1, std_steer=np.radians(1), std_range=0.3, std_bearing=0.1)
    plt.show()
    print('Final P:', _ekf.P.diagonal())
    return (landmarks_1,)
...
@app.cell
def _(landmarks_1, run_localization):
    _ekf = run_localization(landmarks_1[0:2], std_vel=1e-10, std_steer=1e-10, std_range=1.4, std_bearing=0.05)
    print('Final P:', _ekf.P.diagonal())
    return
...
@app.cell
def _(landmarks_1, run_localization):
    _ekf = run_localization(landmarks_1[0:1], std_vel=1e-10, std_steer=1e-10, std_range=1.4, std_bearing=0.05)
    print('Final P:', _ekf.P.diagonal())
    return
...
@app.cell
def _(array, np, run_localization):
    landmarks_2 = array([[5, 10], [10, 5], [15, 15], [20, 5], [15, 10], [10, 14], [23, 14], [25, 20], [10, 20]])
    _ekf = run_localization(landmarks_2, std_vel=0.1, std_steer=np.radians(1), std_range=0.3, std_bearing=0.1, ylim=(0, 21))
    print('Final P:', _ekf.P.diagonal())
    return
