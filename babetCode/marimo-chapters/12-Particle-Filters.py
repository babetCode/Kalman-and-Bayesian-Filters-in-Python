...
def _(np, randn, uniform):
    ...
    return create_gaussian_particles, create_uniform_particles
...
@app.cell
def _(create_uniform_particles, np):
...
def _(np, randn):
    def predict(particles, u, std, dt=1.0):
        ...
        N = len(particles)
        particles[:, 2] = particles[:, 2] + (u[0] + randn(N) * std[0])
        particles[:, 2] = particles[:, 2] % (2 * np.pi)  # update heading
        dist = u[1] * dt + randn(N) * std[1]
        particles[:, 0] = particles[:, 0] + np.cos(particles[:, 2]) * dist
        particles[:, 1] = particles[:, 1] + np.sin(particles[:, 2]) * dist  # move in the (noisy) commanded direction
...
def _(np, scipy):
    def update(particles, weights, z, R, landmarks):
        for i, landmark in enumerate(landmarks):
            ...
            weights = weights * scipy.stats.norm(distance, R).pdf(z[i])
        weights = weights + 1e-300
        weights = weights / sum(weights)  # avoid round-off to zero  # normalize
...
@app.cell
def _(create_uniform_particles, estimate, np):
...
def _(
    ...
    def run_pf1(N, iters=18, sensor_std_err=0.1, do_plot=True, plot_particles=False, xlim=(0, 20), ylim=(0, 20), initial_x=None):
        ...
        if plot_particles:
            alpha = 0.2
            if N > 5000:
                alpha = alpha * (np.sqrt(5000) / np.sqrt(N))
        ...
        for x in range(iters):
            ...
            predict(particles, u=(0.0, 1.414), std=(0.2, 0.05))
    ...
    return randn, run_pf1, scipy, seed
...
@app.function
def resample_from_index_1(particles, weights, indexes):
    particles[:] = particles[indexes]
    weights.resize(len(particles))
    weights.fill(1.0 / len(weights))
...
def _():
    ...
    return (plot_multinomial_resample,)
...
def _(np, random, w):
    def residual_resample(weights):
        ...
        k = 0  # take int(N*w) copies of each weight
        for i in range(N):
        ...
        cumulative_sum[-1] = 1.0  # use multinormial resample on the residual to fill up the rest.
        indexes[k:N] = np.searchsorted(cumulative_sum, random(N - k))  # get fractional part
        return indexes  # normalize  # ensures sum is exactly one
...
def _():
    ...
    return (plot_stratified_resample,)
...
def _(np, random):
    def stratified_resample(weights):
        N = len(weights)
        positions = (random(N) + range(N)) / N  # make N subdivisions, chose a random position within each one
        indexes = np.zeros(N, 'i')
        cumulative_sum = np.cumsum(weights)
        i, j = (0, 0)
        while i < N:
...
def _():
    ...
    return (plot_systematic_resample,)
...
def _(
    np,
    plot_multinomial_resample,
    plot_residual_resample,
    plot_stratified_resample,
    plot_systematic_resample,
):
    a = [.1, .2, .3, .4, .2, .3, .1]
    np.random.seed(4)
