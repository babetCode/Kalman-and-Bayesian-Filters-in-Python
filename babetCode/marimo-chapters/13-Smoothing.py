...
def _(plt):
    ...
    def plot_rts(noise, Q=0.001, show_velocity=False):
        ...
        fk.x = np.array([0.0, 1.0])
        fk.F = np.array([[1.0, 1.0], [0.0, 1.0]])
        fk.H = np.array([[1.0, 0.0]])  # state (x and dx)
        fk.P *= 10.0
        fk.R = noise
        fk.Q = Q_discrete_white_noise(dim=2, dt=1.0, var=Q)  # state transition matrix
        zs = np.asarray([t + randn() * noise for t in range(40)])
        mu, cov, _, _ = fk.batch_filter(zs)  # Measurement function
        M, P, C, _ = fk.rts_smoother(mu, cov)  # covariance matrix
        if show_velocity:  # state uncertainty
            index = 1  # process uncertainty
            print('gu')
        else:  # create noisy data
            index = 0
        if not show_velocity:
            bp.plot_measurements(zs, lw=1)  # filter data with Kalman filter, than run smoother on it
        ...
        plt.show()
    plot_rts(7.0)
    return KalmanFilter, Q_discrete_white_noise, np, plot_rts, random
...
@app.cell
def _(KalmanFilter, Q_discrete_white_noise, np, plt, random):
    ...
    kf_res = abs(kf_x[:, 0] - nom)  # size of lag
