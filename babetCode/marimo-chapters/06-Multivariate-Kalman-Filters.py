...
def _():
    ...
    def compute_dog_data(z_var, process_var, count=1, dt=1.0):
        ...
        return (np.array(_xs), np.array(zs))

    return compute_dog_data, np, randn
...
def _(A, np, x_3):
    ...
    print(A @ x_3)
    print()
    x_4 = np.array([[10.0, 4.5]]).T
    print(A @ x_4)
    print()
    x_4 = np.array([10.0, 4.5])
    print(A @ x_4)
    return
...
@app.cell
def _(np):
    P_1 = np.array([[500.0, 0.0], [0.0, 49.0]])
    P_1
    return
...
def _(dt, np):
    ...
    x_6 = np.array([10.0, 4.5])
    P_2 = np.diag([500, 49])
    F_1 = np.array([[1, dt], [0, 1]])
    x_6, P_2 = predict(x=x_6, P=P_2, F=F_1, Q=0)
    # Q is the process noise
    print('x =', x_6)
    return F_1, P_2, predict, x_6
...
@app.cell
def _(F_1, P_2, predict, x_6):
    for _ in range(4):
        x_7, P_3 = predict(x=x_6, P=P_2, F=F_1, Q=0)
        print('x =', x_7)
    return (P_3,)
...
def _(np, predict):
    ...
    dt_1 = 0.3
    F_2 = np.array([[1, dt_1], [0, 1]])
    x_8 = np.array([10.0, 4.5])
    P_4 = np.diag([500, 500])
    plot_covariance_ellipse(x_8, P_4, edgecolor='r')
    x_8, P_4 = predict(x_8, P_4, F_2, Q=0)
    plot_covariance_ellipse(x_8, P_4, edgecolor='k', ls='dashed')
    return F_2, P_4, plot_covariance_ellipse, x_8
...
def _():
    ...
    print(Q)
    return Q, Q_discrete_white_noise
...
@app.cell
def _(F_2, P_4, Q, predict, x_8):
    B = 0.0  # my dog doesn't listen to me!
    u = 0
    x_9, P_5 = predict(x_8, P_4, F_2, Q, B, u)
    print('x =', x_9)
    print('P =', P_5)
    return B, P_5, u, x_9
...
@app.cell
def _(B, F_2, P_5, Q, predict, u, x_9):
    predict(x_9, P_5, F_2, Q)[0] == predict(x_9, P_5, F_2, Q, B, u)[0]
    return


@app.cell
def _(B, F_2, P_5, Q, predict, u, x_9):
    predict(x_9, P_5, F_2, Q)[1] == predict(x_9, P_5, F_2, Q, B, u)[1]
    return
...
def _(H, P_5, R, x_9):
    ...
    _z = 1.0
    x_10, P_6 = update(x_9, P_5, _z, R, H)
    print('x =', x_10)
    return
...
def _(KalmanFilter, Q_discrete_white_noise, np):
    def pos_vel_filter(x, P, R, Q=0.0, dt=1.0):
        ...
        kf.H = np.array([[1.0, 0]])
        kf.R = kf.R * R
        if np.isscalar(P):  # location and velocity
            kf.P = kf.P * P
        else:  # state transition matrix
            kf.P[:] = P  # Measurement function
        if np.isscalar(Q):  # measurement uncertainty
            kf.Q = Q_discrete_white_noise(dim=2, dt=dt, var=Q)
        else:  # covariance matrix 
            kf.Q[:] = Q
        return kf  # [:] makes deep copy

    return (pos_vel_filter,)
...
@app.cell
def _(np, pos_vel_filter):
    dt_2 = 0.1
    x_11 = np.array([0.0, 0.0])
    kf = pos_vel_filter(x_11, P=500, R=5, Q=0.1, dt=dt_2)
    return (kf,)
...
@app.cell
def _(compute_dog_data, np, pos_vel_filter):
    ...
    def run(x0=(0.0, 0.0), P=500, R=0, Q=0, dt=1.0, track=None, zs=None, count=0, do_plot=True, **kwargs):
        ...
        """
        if zs is None:
            _track, zs = compute_dog_data(R, Q, _count)
        kf = pos_vel_filter(x0, R=R, P=P, Q=Q, dt=dt)
        _xs, cov = ([], [])  # Simulate dog if no data provided. 
        for _z in zs:
            kf.predict()
            kf.update(_z)
            _xs.append(kf.x)  # create the Kalman filter
            cov.append(kf.P)
        _xs, cov = (np.array(_xs), np.array(cov))
        if do_plot:  # run the kalman filter and store the results
            plot_track(_xs[:, 0], _track, zs, cov, **kwargs)
        return (_xs, cov)

    return plot_track, run
...
@app.cell
def _(np, run):
    P_7 = np.diag([500.0, 49.0])
    Ms, _Ps = run(count=50, R=10, Q=0.01, P=P_7)
    return
...
def _(np, run):
    ...
    P_8 = np.diag([3.0, 1.0])
    np.random.seed(3)
    Ms_1, _Ps = run(count=25, R=10, Q=0.01, P=P_8, do_plot=False)
    with figsize(x=9, y=5):
        plot_gaussians(Ms_1[::7], _Ps[::7], (-5, 25), (-5, 5), 75)
    return (P_8,)
...
@app.cell
def _(P_8, Q, R, pos_vel_filter):
    from filterpy.common import Saver
    kf_1 = pos_vel_filter([0, 0.1], R=R, P=P_8, Q=Q, dt=1.0)
    s = Saver(kf_1)
    for i in range(1, 6):
        kf_1.predict()
        kf_1.update([i])
...
@app.cell
def _(book_plots, np, plot_covariance_ellipse):
    dt_3 = 0.6
    x_12 = np.array([0.0, 5.0])
    F_3 = np.array([[1.0, dt_3], [0, 1.0]])
    P_9 = np.array([[1.5, 0], [0, 3.0]])
    plot_covariance_ellipse(x_12, P_9, edgecolor='r')
    for _ in range(5):
        x_12 = F_3 @ x_12
        P_9 = F_3 @ P_9 @ F_3.T
        plot_covariance_ellipse(x_12, P_9, edgecolor='k', ls='dashed')
...
@app.cell
def _(Q_discrete_white_noise, np):
    dt_4 = 1.0
    R_var = 10
    Q_var = 0.01
    x_13 = np.array([[10.0, 4.5]]).T
    P_10 = np.diag([500, 49])
    F_4 = np.array([[1, dt_4], [0, 1]])
    H_1 = np.array([[1.0, 0.0]])
    R_1 = np.array([[R_var]])
    Q_1 = Q_discrete_white_noise(dim=2, dt=dt_4, var=Q_var)
    return F_4, H_1, P_10, Q_1, Q_var, R_1, R_var, x_13
...
def _(
    ...
    _count = 50
    _track, zs = compute_dog_data(R_var, Q_var, _count)
    _xs, cov = ([], [])
    for _z in zs:
        x_14 = F_4 @ x_13
        P_11 = F_4 @ P_10 @ F_4.T + Q_1
        S = H_1 @ P_11 @ H_1.T + R_1
        K = P_11 @ H_1.T @ inv(S)
        y = _z - H_1 @ x_14
        x_14 = x_14 + K @ y
        P_11 = P_11 - K @ H_1 @ P_11
        _xs.append(x_14)
        cov.append(P_11)
    _xs, cov = (np.array(_xs), np.array(cov))
    plot_track(_xs[:, 0], _track, zs, cov, plot_P=False)
    return (P_11,)
...
@app.cell
def _(KalmanFilter, np, plt, pos_vel_filter, randn):
    from math import sqrt

    def univariate_filter(x0, P, R, Q):
        _f = KalmanFilter(dim_x=1, dim_z=1, dim_u=1)
        _f.x = np.array([[x0]])
        _f.P = _f.P * P
        _f.H = np.array([[1.0]])
        _f.F = np.array([[1.0]])
    ...
    def plot_1d_2d(xs, xs1d, xs2d):
        ...
        plt.plot(_xs, ls='--', color='k', lw=1, label='track')
        plt.title('State')
        plt.legend(loc=4)
        plt.show()
...
@app.cell
def _(P_11, compute_dog_data, run):
    from numpy.random import seed
    seed(2)
    trk, zs_1 = compute_dog_data(z_var=225, process_var=0.02, count=50)
    run(track=trk, zs=zs_1, R=225, Q=200, P=P_11, plot_P=False, title='R_var = 225 $m^2$, Q_var = 20 $m^2$')
    run(track=trk, zs=zs_1, R=225, Q=0.02, P=P_11, plot_P=False, title='R_var = 225 $m^2$, Q_var = 0.02 $m^2$')
    return seed, trk, zs_1
...
@app.cell
def _(P_11, run, trk, zs_1):
    run(track=trk, zs=zs_1, R=10000, Q=0.2, P=P_11, plot_P=False, title='R=$10,000\\, m^2$, Q=$.2\\, m^2$')
    return
...
@app.cell
def _(P_11, np, run, trk, zs_1):
    run(track=trk, zs=zs_1, R=10000, Q=0.2, P=P_11, plot_P=False, x0=np.array([50.0, 1.0]), title='R=$10,000\\, m^2$, Q=$.2\\, m^2$')
    return
...
@app.cell
def _(P_11, np, run, trk, zs_1):
    run(track=trk, zs=zs_1, R=225, Q=0.2, P=P_11, plot_P=False, x0=np.array([20.0, 1.0]), title='R=$225\\, m^2$, Q=$.2\\, m^2$')
    return
...
def _(run, trk, zs_1):
    ...
    run(track=trk, zs=zs_1, R=var, Q=0.02, P=500.0, plot_P=True, title='$P=500\\, m^2$')
    return (var,)
...
@app.cell
def _(run, trk, var, zs_1):
    run(track=trk, zs=zs_1, R=var, Q=0.02, P=1.0, plot_P=True, title='$P=1\\, m^2$')
    return
...
@app.cell
def _(np, run, trk, var, zs_1):
    x_15 = np.array([100.0, 0.0])
    run(track=trk, zs=zs_1, R=var, Q=0.02, P=1.0, x0=x_15, plot_P=False, title='$P=1\\, m^2$')
    return
...
@app.cell
def _(np, run, trk, var, zs_1):
    x_16 = np.array([100.0, 0.0])
    run(track=trk, zs=zs_1, R=var, Q=0.02, P=500.0, x0=x_16, plot_P=False, title='$P=500\\, m^2$')
    return
...
def _(compute_dog_data, plt, pos_vel_filter):
    ...
    def plot_covariances(count, R, Q=0, P=20.0, title=''):
        ...
        for _z in zs:
            ...
            _xs.append(_f.x[0])
            cov.append(_f.P)
        plot_track_ellipses(_count, zs, _xs, cov, title)
...
@app.cell
def _(book_plots, compute_dog_data, np, plt, pos_vel_filter):
    _count = 50
    _track, zs_2 = compute_dog_data(10, 0.2, _count)
    P_12 = np.diag([500.0, 49.0])
    _f = pos_vel_filter(x=(0.0, 0.0), R=3.0, Q=0.02, P=P_12)
    _xs, _, _, _ = _f.batch_filter(zs_2)
    book_plots.plot_measurements(range(1, _count + 1), zs_2)
    book_plots.plot_filter(range(1, _count + 1), _xs[:, 0])
    plt.legend(loc='best')
    return
...
@app.cell
def _(Saver, compute_dog_data, np, plt, pos_vel_filter):
    _track, zs_3 = compute_dog_data(10, 0.2, 200)
    P_13 = np.diag([500.0, 49.0])
    _f = pos_vel_filter(x=(0.0, 0.0), R=3.0, Q=0.02, P=P_13)
    s_1 = Saver(_f)
    _xs, _, _, _ = _f.batch_filter(zs_3, saver=s_1)
    s_1.to_array()
    plt.plot(s_1.y)
    return
...
@app.cell
def _(book_plots, compute_dog_data, np, plt, pos_vel_filter, seed):
    _count = 50
    seed(8923)
    P_14 = np.diag([500.0, 49.0])
    _f = pos_vel_filter(x=(0.0, 0.0), R=3.0, Q=0.02, P=P_14)
    _track, zs_4 = compute_dog_data(3.0, 0.02, _count)
    Xs, Covs, _, _ = _f.batch_filter(zs_4)
    Ms_2, _Ps, _, _ = _f.rts_smoother(Xs, Covs)
    book_plots.plot_measurements(zs_4)
    plt.plot(Xs[:, 0], ls='--', label='Kalman Position')
    plt.plot(Ms_2[:, 0], label='RTS Position')
    plt.legend(loc=4)
    return Ms_2, Xs
...
def _(Ms_2, Xs, plt):
    ...
    plt.plot(Ms_2[:, 1], label='RTS Velocity')
    plt.legend(loc=4)
