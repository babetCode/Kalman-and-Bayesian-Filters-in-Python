...
def _():
    ...
    from filterpy.stats import covariance_ellipse, plot_covariance_ellipse
    mean = (5, 3)
    _P = np.array([[32, 15], [15.0, 40.0]])
    _x, y = multivariate_normal(mean=mean, cov=_P, size=2500).T
    plt.scatter(_x, y, alpha=0.3, marker='.')
    plt.axis('equal')
    plot_covariance_ellipse(mean=mean, cov=_P, variance=2.0 ** 2, facecolor='none')
    return np, plt
...
def _(np, plt):
    ...
    _x = np.array([0.0, 1.0])
    _P = np.eye(2) * 100.0
    enf = EnKF(x=_x, P=_P, dim_z=1, dt=1.0, N=20, hx=hx, fx=fx)
    std_noise = 10.0
    enf.R *= std_noise ** 2
    ...
    for t in range(0, 100):
        ...
        kf_results.append(kf.x[0, 0])  # create measurement = t plus white noise
    ...
    plt.fill_between(range(100), results - ps, results + ps, facecolor='y', alpha=0.3)  # save data
    plt.legend(loc='best')
    return
